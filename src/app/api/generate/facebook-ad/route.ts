...
import { createFacebookAdPrompt } from "@/lib/prompts";
...
export async function POST(req: Request) {
  ...
  const parsedRequest = facebookAdRequestSchema.safeParse(
  ...
  const result = streamText({
    ...
    `,
    prompt: createFacebookAdPrompt(parsedRequest.data),
    output: Output.object({
      schema: facebookAdOutputSchema,
    }),
  });
